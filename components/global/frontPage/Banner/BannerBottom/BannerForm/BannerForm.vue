<template>
    ...
    >
        <div class="banner___bottom-contact-form-wrapper border-dec">
            <h3
                    v-show="contactForm.title"
                    class="contact-form__title banner__bottom-contact-form-title"
            >
                {{ contactForm.title }}
            </h3>
            <div
                    v-show="contactForm.subtitle"
                    class="contact-form__subtitle banner__bottom-contact-form-subtitle"
            >{{ contactForm.subtitle }}</div>
            <form class="contact-form__container banner__bottom-contact-form__container" @submit.prevent="sendForm">
...
<script>
    ...
    export default {
        async fetch() {
            ...
            this.contactForm = _get(data, "nodeByUri.mainPage.banner.contactForm", {})
        },
        data(){
            return{
                contactForm: {},
        ...
        watch:{
          name: function () {
              this.$v.name.checkName?this.$refs.bannerName.classList.add('done'):''
          },
        },
        validations: {
            tel: {
